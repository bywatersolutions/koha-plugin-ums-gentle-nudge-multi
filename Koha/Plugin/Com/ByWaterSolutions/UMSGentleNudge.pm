...
BEGIN {
    my $path = Module::Metadata->find-module_by_name(_PACKAGE_);
    $path =~ s!\.pm$!/lib!;
    unshift @INC, $path;
    ...
    Koha::Schema->register_class(KohaPluginComBywaterSolutionsUMSConfig => 'Koha::Schema::result::KohaPluginComBywatersolutionsUMSConfig');
...
sub configure {
    my ( $self, $args ) = @_;
    my $cgi = $self->{'cgi'};
    my $config_count = Koha::UMSConfigs->search()->count;
        my $template = $self->get_template( { file => 'newconfig.tt' } );
